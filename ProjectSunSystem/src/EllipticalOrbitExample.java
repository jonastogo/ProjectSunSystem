...
public class EllipticalOrbitExample extends JFrame {
	...
	Runnable renderLoop = new Runnable() {
		...
		public void run() {
			...
			Image sun = null;
			try {
				sun = ImageIO.read(new URL("http://i.imgur.com/85mYqQy.png")).getScaledInstance(sunPoint.x, sunPoint.y,
			...
			try {
				earth = ImageIO
						.read(new URL(
								"http://img1.wikia.nocookie.net/__cb20131031215453/dc-cinematic-universe/de/images/e/e1/Erde.png"))
						.getScaledInstance(earthPoint.x, earthPoint.y, BufferedImage.SCALE_SMOOTH);
			} catch (Exception e) {
				e.printStackTrace();
			}
			Image moon = null;
			try {
				moon = ImageIO
						.read(new URL(
								"http://orig13.deviantart.net/957e/f/2011/363/9/9/moon_png_by_natyjonasproductions-d4kod97.png"))
						.getScaledInstance(moonPoint.x, moonPoint.y, BufferedImage.SCALE_SMOOTH);
			} catch (Exception e) {
				e.printStackTrace();
			}
			double angle = 0;
			...
			while (true) {
				...
				g.drawOval((size.x / 2) - (ovalPoint.x / 2), (size.y / 2) - (ovalPoint.y / 2), ovalPoint.x,
						ovalPoint.y);
				...
				int mx=(int) (Math.cos(angle)*(earthPoint.x+20)+x);
				int my=(int) (Math.cos(angle)*(earthPoint.y+20)+y);
